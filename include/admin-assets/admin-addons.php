...
//     echo 'Powered by <a href="http://www.wordpress.org" target="_blank">WordPress</a> | Theme Developer <a href="http://site-np.esy.es" target="_blank">Nazar Pelekh</a>';
// }
// add_filter('admin_footer_text', 'remove_footer_admin');

// Login area branding styles
function wp_login_candy() {
    wp_enqueue_style( 'wpcandy', theme('include/admin-assets/admin-area.css'), false );
}
...
function tt_media_add_js($hook) {
    ...
    if ( $hook != 'upload.php' )
        return;
    wp_enqueue_script( 'tt_media_js', theme('include/admin-assets/admin-area.js'), array('jquery'), 1.0, true );
